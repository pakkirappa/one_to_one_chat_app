...
export const SocketProvider = ({ children }) => {
  ...
  useEffect(() => {
    if (user) {
      ...
      console.log('Setting up socket connection for user:', user.username, 'Token exists:', !!token);
      ...
      newSocket.on('connect', () => {
        console.log('Socket connected successfully for user:', user.username, 'Socket ID:', newSocket.id);
        setSocket(newSocket);
      });
      ...
      newSocket.on('connect_error', (error) => {
        console.error('Socket connection error for user:', user.username, error);
      ...
      newSocket.on('newMessage', (data) => {
        ...
        console.log('Received new message via Socket for user:', user.username, 'Message from:', message.sender.username, 'To:', message.receiver.username);
        ...
        if (message.sender._id !== user.id) {
          const messageText = message.messageType && message.messageType !== 'text' 
      ...
      newSocket.on('messageSent', (data) => {
        ...
        console.log('Message sent confirmation received for user:', user.username, 'tempId:', tempId, 'message:', message._id);
  ...
  const sendMessage = (receiverId, content, messageType = 'text') => {
    console.log('sendMessage called with:', { receiverId, content, messageType, socketConnected: !!socket, userId: user?.id });
