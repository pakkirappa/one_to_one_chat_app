...
const ChatWindow = ({ conversation, messages, currentUser }) => {
  ...
  const handleFileUploaded = useCallback((messageData) => {
    ...
    if (conversation && conversation.isTemp && messageData.conversationId) {
      console.log('Joining real conversation room after file upload:', messageData.conversationId);
  ...
  const getFilteredMessages = () => {
    ...
    const filtered = messages.filter(message => 
      (message.sender._id.toString() === currentUserId && message.receiver._id.toString() === otherUserId) ||
      (message.sender._id.toString() === otherUserId && message.receiver._id.toString() === currentUserId)
    );
  ...
  const isTypingInThisConversation = () => {
    ...
    return Object.values(typingUsers).some(typing => 
        ...
        <div className="d-flex align-items-center">
          ...
          <div>
            ...
            <small className="text-muted">
              {otherUser?.isOnline ? (
                <Badge bg="success" className="me-1">Online</Badge>
              ) : otherUser?.lastSeen ? (
                ...
                >
                  {message.messageType && message.messageType !== 'text' ? (
                    ...
                    >
                      ...
                      <div className={`mt-1 text-end ${isOwn ? 'text-light' : 'text-muted'}`} 
              ...
              <div className="bg-white border rounded-3 px-3 py-2 shadow-sm">
                <small className="text-muted fst-italic">
                  ...
                  <span className="ms-1">
                    ...
                    <span className="spinner-grow spinner-grow-sm me-1" style={{ width: '0.4rem', height: '0.4rem' }}></span>
                    <span className="spinner-grow spinner-grow-sm" style={{ width: '0.4rem', height: '0.4rem' }}></span>
                  </span>
                </small>
              </div>
            </div>
