...
const Sidebar = ({
  ...
  const { onlineUsers, messages, typingUsers } = useSocket();
  ...
  const getUnreadCount = (conversation) => {
    ...
    // Only count messages from the other user that are unread
    return messages.filter(
      (msg) =>
        msg.sender._id.toString() === otherUser._id.toString() &&
        msg.receiver._id.toString() === currentUser.id.toString() &&
        !msg.isRead &&
        !msg.isTemp
    ).length;
  };

  // Check if the conversation has the latest message
  const hasUnreadMessages = (conversation) => {
    return getUnreadCount(conversation) > 0;
  };

  // Check if user is typing in this conversation
  const isUserTypingInConversation = (conversation) => {
    if (!conversation || !typingUsers) return false;
    ...
    return Object.values(typingUsers).some(
      (typing) =>
        ...
        typing.userId === otherUser._id.toString()
    );
  };
              ...
              <>
                ...
                <ListGroup variant="flush">
                  {conversations.map((conversation) => {
                    ...
                    const isTyping = isUserTypingInConversation(conversation);
                    ...
                    return (
                      <ListGroup.Item
                        ...
                        className={`d-flex align-items-center py-3 ${
                          isUnread ? "fw-bold unread-conversation" : ""
                        }`}
                      >
                        <div className="position-relative me-3">
                          <div
                        ...
                        <div className="flex-grow-1 min-width-0">
                          <div className="d-flex justify-content-between align-items-start">
                            ...
                            {conversation.lastMessage && !isTyping && (
                              <small
                                className={`${
                                  isUnread ? "text-danger" : "text-muted"
                                }`}
                              >
                          ...
                          {isTyping ? (
                            ...
                            >
                              ...
                              conversation.lastMessage.messageType !== "text"
                                ? `Sent a ${conversation.lastMessage.messageType}`
